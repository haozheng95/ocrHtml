...
from aiapp.chinese_ocr.ctpn.lib.fast_rcnn.config import cfg_from_file
from aiapp.chinese_ocr.ctpn.text_detect import text_detect
from aiapp.chinese_ocr.densenet.model import predict as keras_densenet
...
sys.path.append(os.path.join(base_path, "ctpn"))
...
def model(img, adjust=False):
    ...
    @adjust: 是否调整文字识别结果
    """
    cfg_from_file(os.path.join(base_path, 'ctpn/ctpn/text.yml'))
...
def export(image_file, image_output_file, output_file):
    with open(output_file, "w", encoding="utf-8") as f:
        raw = image_output_file
        print(image_file)
        image = np.array(Image.open(image_file).convert('RGB'))
        t = time.time()
        result, image_framed = model(image)
        image_output_file = os.path.join(raw, image_file.split('/')[-1])
        Image.fromarray(image_framed).save(image_output_file)
